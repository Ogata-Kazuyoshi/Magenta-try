...
import { wheelSpeedFL } from './testData.ts';

(window as any).global = window;
import { useState } from 'react';
import { MusicRNN, Player, sequences, NoteSequence } from '@magenta/music';
import { wheelSpeedFL2 } from './testData2.ts';
import {accelePadalAngle} from "./testData3.ts";

export const Magenta: React.FC = () => {
...
const timeSeriesToNoteSequence = (data: number[]): NoteSequence => {
  ...
  //     partInfos: [],
  //     textAnnotations: [], // 追加
  //     sectionAnnotations: [], // 追加
  //     sectionGroups: [], // 追加
  //     toJSON: () => ({}), // 追加
  //     collectionName: '', // 追加
  //   };

  //   const notes: NoteSequence.INote[] = data.map((value, index) => {
  //     const pitch = Math.max(0, Math.min(127, 60 + Math.floor(value * 12))); // ピッチを0から127の範囲に制限
  //     return {
  //       pitch,
  //       startTime: index * 0.5, // 時間を設定 (横軸)
  //       endTime: (index + 1) * 0.5, // 時間を設定 (横軸)
  //       velocity: Math.max(0, Math.min(127, Math.floor(value * 127))), // ベロシティを0から127の範囲に制限
  //     };
  //   });
  ...
  //   const notes: NoteSequence.INote[] = data.map((value, index) => {
  //     // ピッチを0から127の範囲に制限し、ランダム性を追加
  //     const pitch = Math.max(
  //       0,
  //       Math.min(
  //         127,
  //         60 + Math.floor(value * 12) + Math.floor(Math.random() * 12 - 6)
  //       )
  //     );
  //     return {
  //       pitch,
...
const getTimeSeriesData = (): number[] => {
  ...
  return accelePadalAngle.map((elm) => elm * 0.0001);
